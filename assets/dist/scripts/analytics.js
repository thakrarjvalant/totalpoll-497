        ...
        (function (Providers) {
            var ColorsService = /** @class */ (function () {
                function ColorsService() {
                    this.colors = ["#006595","#00a5b5","#00b259","#949ca1","#0593f2","#ba3725","#ed890e","#0eed37","#673ab7", "#3f51b5","#472f2f","#f20519","#1ba1e2", "#0050ef", "#6a00ff", "#aa00ff", "#f472d0", "#d80073", "#a20025", "#e51400", "#fa6800", "#f0a30a", "#e3c800", "#825a2c", "#6d8764", "#647687", "#76608a", "#a0522d"];
                }
                ColorsService.prototype.getRandomColors = function (count,shuffle) {
                    if(shuffle){
                ...
                function Chart() {
                    return {
                        ...
                        link: function ($scope, element, attributes, ngModel) {
                                    ...
                                    tooltips: {
                                            callbacks: {
                                                label: function(tooltipItem, data) {
                                                    ...
                                                    return [tooltipLabel + ', ' + tooltipData + ', ' + tooltipPercentage + '%','Total:'+total];
                                                }
                                            }
                                        }
                                },
                            ...
                            if (defaultOptions.type === 'doughnut' || defaultOptions.type === 'pie') {
                                delete defaultOptions.options.scales;
                            }else if (defaultOptions.type === 'bar') {
                                delete defaultOptions.options.tooltips;//removing customized tooltip for bar chart
                            }
    ...
    var RepositoryService = /** @class */ (function () {
        ...
        RepositoryService.prototype.getMetrics = function (filters) {
            ...
            return this.resource
                .metrics(angular.extend({}, { action: 'totalpoll_analytics_metrics' }, filters))
                .$promise
                .then(function (response) {
                var metrics = { choice: {}, day: {}, month: {}, year: {}, os: {}, browser: {},votes:{},resolution:{} };

                var vote_table = res_table = '';
                var totol_voting = 0;
                //added by BK for analytics - for votes
                angular.forEach(response.votes, function (item) {
                    metrics.votes[item.label] = Number(item.count);
                    vote_table += '<tr><td>'+item.label+'</td><td>'+item.count+'</td></tr>';
                    totol_voting += parseInt(item.count);
                });
                vote_table += '<tr><td><b>Total Voting</b></td><td><b>'+totol_voting+'</b></td></tr>';
                angular.forEach(response.resolution, function (item) {
                    metrics.resolution[item.label] = Number(item.count);
                    var value =  parseFloat(item.count);
                    value = value.toLocaleString(undefined, { maximumFractionDigits: 7, minimumFractionDigits: 0 });
                    value = (value >= 99.9999999) ? '100.0000000' : value;
                    res_table += '<tr><td>'+item.label+'</td><td>'+ value +'%</td></tr>';
                });
                setTimeout(function () {
                  jQuery('.poll-chart-title').html("<center><h2>"+jQuery(".poll-dropdown option:selected").html()+"</h2></center>");
                  jQuery('#resolution-table').html('<table cellpadding="5">'+res_table+'</table>');
                  jQuery('#vote-count-table').html('<table cellpadding="5">'+vote_table+'</table>');
                }, 0);

                angular.forEach(metrics, function (value, key) {
                    var color = _this.ColorsService.getRandomColors(1, 0)[0];
                    metrics[key] = {
                    ...
                    metrics[key].datasets[0].backgroundColor = _this.ColorsService.getRandomColors(metrics[key].labels.length, 0);
        ...
        RepositoryService.prototype.getPolls = function () {
            return this.resource.polls({ action: 'totalpoll_analytics_polls' }).$promise;
        };
...
(function (TotalPoll) {
    ...
    var AnalyticsBrowserComponent = /** @class */ (function () {
        function AnalyticsBrowserComponent(RepositoryService, $location) {
        ...
        AnalyticsBrowserComponent.prototype.$onInit = function () {
            this.loadPolls();
            if (this.filters.poll) {
                this.loadMetrics();
            }
        };
        AnalyticsBrowserComponent.prototype.isProcessing = function () {
            return this.processing;
        };
        AnalyticsBrowserComponent.prototype.loadMetrics = function () {
            var _this = this;
            if (!this.filters.poll) {
                this.updateUrl();
                this.metrics = {};
            ...
            setTimeout(function () {
              if(jQuery('.totalpoll-analytics-charts').length){
                jQuery('.exp-btn-div').show();
              }else{
                jQuery('.exp-btn-div').hide();
              }
            }, 0);
        };
        AnalyticsBrowserComponent.prototype.loadPolls = function () {
            var _this = this;
            this.processing = true;
            this.RepositoryService
                .getPolls()
        ...
        AnalyticsBrowserComponent.prototype.resetFilters = function () {
            this.filters.from = null;
            this.filters.to = null;
            this.loadMetrics();
        };
        AnalyticsBrowserComponent.prototype.updateUrl = function () {
            this.$location.search(angular.extend({}, this.$location.search(), this.filters));
        };
        AnalyticsBrowserComponent = __decorate([
            Component('components.totalpoll', {
                templateUrl: 'analytics-browser-component-template',
                bindings: {}
            })
        ], AnalyticsBrowserComponent);
        return AnalyticsBrowserComponent;
    }());
})(TotalPoll || (TotalPoll = {}));
//@PRO
...
(function (TotalPoll) {
    ...
    TotalPoll.analytics = angular
        ...
        .value('pollId', window['TotalPollAnalytics']['pollId'] || null);
})(TotalPoll || (TotalPoll = {}));
//@END-PRO
